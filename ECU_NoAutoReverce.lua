...
function onTick()
	...
	if throttle == 1 then
		if GEAR == 0 then
			...
			if REVERSE_CHECKER == 0 and GEAR_REVERSE then
				GEAR_REVERSE = false
			end
			BRAKE = 1
		else
	...
	elseif throttle == -1 then
		if GEAR == 0 then
			THROTTLE_OUT = THROTTLE_PERCENTAGE / 100
			if REVERSE_CHECKER == 0 and (not GEAR_REVERSE) then
				GEAR_REVERSE = true
			end
			BRAKE = 1
		else
	...
	if ENGINE_RPS > REV_LIMIT_RPS then
		THROTTLE_OUT = 0
	end
	if throttle ~= 0 then
		REVERSE_CHECKER = REVERSE_CHECKER + 1
	else
		REVERSE_CHECKER = 0
	end
	...
	output.setBool(1, GEAR_ORDER_ARRAY[GEAR+1][1])
	output.setBool(2, GEAR_ORDER_ARRAY[GEAR+1][2])
	output.setBool(3, GEAR_ORDER_ARRAY[GEAR+1][3])
	output.setBool(4, GEAR_ORDER_ARRAY[GEAR+1][4])
